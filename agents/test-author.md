...
- Read `docs/features/<slug>/data-model.md`, `contracts/openapi.yaml`, and the feature's Accepted ADRs in `docs/adr/` for the shapes/contracts the test must assert against.
...
- **Laravel (the team's stack):** PHPUnit (never Pest), Mockery for mocks, Laravel factories for models; tests run on `sqlite :memory:` with `QUEUE_CONNECTION=sync`. Run with `php artisan test --compact` (add a path or `--filter=<method>` for one file / method). **Never make real HTTP / SMTP / channel calls** — fake them (`Queue::fake()`, HTTP fakes, mocked channels). Feature tests → `tests/Feature/`, unit tests → `tests/Unit/`.
