...
- **Trusting `tasks.json`'s `acs` as the complete AC set.** They only list what the diff *claims*. Review traces the **whole** §5 set end-to-end — an AC that never reached the diff (no task wrote it, no test asserts it) is the most dangerous gap precisely because the claimed set can't reveal it.
...
The review-record format (step 5) is defined in the Protocol prose — no separate template file.
