...
description: >
  ...
  Proposed → Accepted review flow. Output: docs/adr/NNNN-{title}.md (committed durable doc).
...
1. **Locate the feature for context.** `test -d docs/features/<slug>`. Missing → ask whether to proceed standalone (`AskUserQuestion`, phrasing per [`../_shared/ask-style.md`](../_shared/ask-style.md)); on «yes», flag that design context is absent. `sad.md` present → read its §4 (strategy), §9 (existing ADR index), §11 (risks); absent → note it and source context from the user. **The ADR itself is written to the committed `docs/adr/`** (a durable doc — `output-discipline`), not under the gitignored feature folder; ensure `docs/adr/` exists (the dev commits it).
...
3. **Dedup.** `ls docs/adr/*.md 2>/dev/null`. An Accepted ADR on the same topic exists → don't duplicate: propose either editing it or a new ADR that marks the old one `Superseded by NNNN` (and stamps the old one's `status` + `updated_at`).
4. **Pick the number.** `NNNN` = (count of existing `docs/adr/*.md`) + 1, zero-padded to 4 digits (`0001`, `0002`, …) — ADRs are **globally numbered** across the repo, not per-feature. Never reuse a number.
5. **Copy the template.** Copy [`../design/templates/adr.md`](../design/templates/adr.md) → `docs/adr/NNNN-<title>.md`. This is the canonical MADR shape, owned by `design` and referenced here — do not invent a variant. Patch frontmatter: `owner`, `updated_at: <today>`, `feature_size` (from `.size` if present), `ticket`.
...
12. **Close the loop.** Add a row to `sad.md` §9 ADR index (and link from `tasks/_epic.md` if the ADR scopes a specific task) — these are gitignored process artifacts pointing *forward* to the committed ADR. The ADR's own `## Links` must point to its **Jira ticket** (the spec/sad are gitignored — reference them via the ticket, not an in-repo link) and any sibling ADR in `docs/adr/` — no orphans. **Then register the decision in the human-facing log** (this fork — [`../_shared/paths.md`](../_shared/paths.md) Rule 2): append/update a one-line row in `docs/ecosystem/design-decisions.md` — decision title in `## N. <Title>` form + `**Status: Decided**` + a link to `docs/adr/NNNN-<title>.md`. The full MADR stays the ADR file; this is just the index `AGENTS.md` cites. Leave both unstaged (§1).
13. **Write + handoff.** Then **emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) (utility variant) — *What I did* + *Review* (`docs/adr/NNNN-<title>.md`, committed — left unstaged for your commit) + *Run next*: resume the gate that needed it (`/sdd:tasks <slug>` or `/sdd:plan-tests <slug>`); `/clear` optional.

## Definition of Done

- `docs/adr/NNNN-<title>.md` exists (committed durable doc, left unstaged) in design's MADR format (frontmatter + Context + Decision drivers + Considered options + Decision outcome + Consequences + Links).
...
- Linked: a row in `sad.md` §9 (when `sad.md` exists, pointing forward to the committed ADR) and the ADR's `## Links` point to its Jira ticket (+ any sibling ADR). A genuinely standalone ADR notes the missing design context instead.
