...
1. `NNNN` = `ls docs/adr/*.md 2>/dev/null | wc -l` + 1, zero-padded to 4 digits — ADRs are **globally numbered** in the committed `docs/adr/`, never per-feature.
...
3. Copy [`../templates/adr.md`](../templates/adr.md) → `docs/adr/NNNN-<title>.md` (a **committed** durable doc, left unstaged for the dev). Status = `Accepted` (this skill is synchronous); Considered options include the rejected ones from the `AskUserQuestion`; no strawman options (an alternative an existing constraint already excludes).
4. Add a §9 row in-memory: `| NNNN | <imperative title> | Accepted | §N |` — the §9 index (in the gitignored `sad.md`) points forward to the committed ADR.
5. Append to an in-memory ADR-spawns log (`{adr_id, title, section, triggered_by}`), kept **adjacent** to the edits-log — different signal (new artifact, not a user edit). The critic reads `docs/adr/` directly; the spawns log is in-memory traceability only.
