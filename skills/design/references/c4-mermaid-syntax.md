...
design emits C4 Level 1 (Context) in §3 and Level 2 (Container) in §5 as Mermaid blocks inline in `sad.md`. L3 Component and L4 Code are deliberately out of scope — request a separate diagramming pass if you need them. Mermaid renders natively in GitHub and most Markdown viewers.
...
In practice: open `sad.md` in a Mermaid-capable Markdown viewer (or on GitHub) and inspect the render — it fails loudly on syntax errors.
