...
- **`{{UPSTREAM_FILES}}`** (the critic Reads these itself — paths only, never bodies):
  ...
  - `docs/adr/` — `ls` it (the committed, globally-numbered ADR dir), then read each ADR's Status / Title / Considered options / Decision outcome.
...
- §9 ADR table is **closed against the feature's ADRs in `docs/adr/`**: every ADR this feature spawned has a §9 row, every §9 row points to an existing file in `docs/adr/` (no orphans either way).
