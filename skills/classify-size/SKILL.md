...
description: >
  ...
  "класифікуй розмір {slug}", "який розмір фічі", "XS чи M". Asks three AskUserQuestion
  (time / new module-API-migration / breaking changes), maps to a size class via
...
2. **Ask the three signals** — one `AskUserQuestion` each, phrased per [`../_shared/ask-style.md`](../_shared/ask-style.md):
...
3. **Map to a class** using the table in [`../_shared/size-matrix.md`](../_shared/size-matrix.md). On an edge case, name the dominant signal aloud («M because it adds a new API + 1–2 sprints, even though it touches only one module»). For an all-maximums answer, ask explicitly «needs a separate roadmap?» → yes = XL.
...
7. **Write + handoff.** Then **emit the stage-handoff block** per [`../_shared/handoff.md`](../_shared/handoff.md) (utility variant) — *What I did* + *Review* (`.size`) + *Run next*: resume your backbone stage (e.g. `/sdd:specify <slug>`); `/clear` optional.
...
- **Optimistic «it's probably S».** Run the three questions — a week later the skipped design hurts.
...
> **Skill:** `.size` absent → asks the three questions (`~1 week`, `one of three — new API`, `internal only`) → maps to **S** (rationale: one API endpoint + ~1 week + internal breaking) → confirms → writes `docs/features/rate-limiting-per-user/.size` = `S` → `spec.md` not yet written, skip sync. (No commit — `.size` is a gitignored process artifact, not committed — §5; left unstaged.)
