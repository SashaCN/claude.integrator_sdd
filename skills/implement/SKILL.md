...
9. **Per-task gate.** After GREEN+REFACTOR: unit + (integration if available) + lint + vet must be clean. Update `tasks/tracker.md` → `done`. **No per-task commit** — this fork commits the whole feature once (step 10), per the commit policy in [`../_shared/handoff.md`](../_shared/handoff.md).
...
- `tasks/tracker.md` reflects final status; the summary reports the gate results and hands off to `review` (the independent review gate) — `implement` does not self-certify the whole change.
