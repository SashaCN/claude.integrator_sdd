...
// Each task is one independent pipeline: write-test → implement → verify → [review]. The feature is committed once, after the workflow returns.
const done = new Set();
...
- Code changes are produced per pipeline stage and merged onto the feature branch; after the workflow returns, the engine **proposes one `[TASK-{JIRA_KEY}]` feature commit** for the whole change — no per-task commits, no `SDD-Task`/`SDD-AC` trailers (AC traceability is in `tasks.json`). It waits for the user's go-ahead (`auto_commit: off`).
