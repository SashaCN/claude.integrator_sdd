...
When the decision tree selects the team, the engine becomes a **lead** coordinating three roles through a shared task list, one git worktree per agent, the lead landing all work as one feature commit. Use this for features with genuine parallel width and a desire for an independent review pass.
...
The lead pulls each agent's worktree changes for a `done` task onto the feature branch as they complete — **no per-task commits**. When every task is gate-clean, the lead **proposes one feature commit** in the team format `[TASK-{JIRA_KEY}] <imperative>` (no `SDD-Task`/`SDD-AC` trailers — AC traceability is in `tasks.json`; see [`tdd-loop.md`](./tdd-loop.md)) and waits for the user's go-ahead before committing.
