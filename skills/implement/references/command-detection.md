...
4. **Language manifests** (the broad fallback — pick the toolchain the manifest implies):
   - **`composer.json` + `artisan` (Laravel — the team's stack).** unit `php artisan test --compact tests/Unit`; integration / feature `php artisan test --compact tests/Feature` (PHPUnit on `sqlite :memory:` with `QUEUE_CONNECTION=sync` — **in-process, needs no Docker**); lint / format `vendor/bin/pint --dirty --format agent`; vet `vendor/bin/phpstan analyse` **only if larastan / phpstan is installed**, else skip. A Laravel repo often also has a `package.json` for frontend assets — **prefer these artisan commands over its scripts**. Tests use PHPUnit (never Pest), Mockery for mocks, Laravel factories for models.
   - `composer.json` without `artisan` (plain PHP) → `vendor/bin/phpunit` / `vendor/bin/phpunit --testsuite=Integration` / `vendor/bin/pint --dirty` / `vendor/bin/phpstan analyse` (if present).
...
5. **Integration tier — Docker probe.** Whatever produced the integration command, confirm a Docker daemon is reachable (`docker info` succeeds) before trusting it — most integration suites spin up an ephemeral dependency (testcontainers-style). Feed the probe result to `require_integration` (see [`settings.md`](./settings.md)): `auto` → run if reachable else NON-red; `always` → BLOCK if unreachable; `never` → skip.
   - **Laravel exception.** The Laravel **Feature** tier runs on `sqlite :memory:` in-process, so it needs **no Docker** — treat it as always-runnable, never gate it behind the Docker probe. `require_integration` only bites when a suite genuinely needs a Docker-backed service (a real MySQL / Redis integration test).
