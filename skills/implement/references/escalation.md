...
3. **Split the task.** If the task turns out to bundle two concerns (e.g. a validation rule *and* an audit write), split it into two with a dep edge, and drive each with its own RED. Update `tasks.json` + `tasks/tracker.md` so the DAG stays the source of truth.
