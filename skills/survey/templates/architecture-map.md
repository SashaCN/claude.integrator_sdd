...
> **Derived cache — not the source of truth.** The curated **`AGENTS.md`** (and the `docs/` it links)
> is the authored source of truth; this file is the *generated index below it* — the **current**
> architecture (what exists today), produced by `survey` and read by specify / design / data-model /
> implement so they don't re-scan the code. It carries the **volatile, regenerable** detail
> (`file:line` anchors, precedent features) that `AGENTS.md` deliberately omits. Gitignored and
> disposable: refresh with `survey` when the repo drifts past `reflects_commit`. On any conflict with
> `AGENTS.md`/`docs/`, the curated docs win and this map is re-derived — never the reverse.
