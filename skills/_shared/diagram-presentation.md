...
> source lands in the file (where a Markdown viewer / GitHub renders it) and, if a renderer is available, an image.
...
1. **Write the diagram to its file first.** The `sad.md` §6 flow, the §3 C4Context block, etc. — that's where a Markdown viewer (or GitHub) renders it natively, so writing-first means the user can flip to the rendered view immediately. (This reverses the old «show then write» order: in practice «show» dumped raw source. Write-first makes the file the render surface.)
...
4. **Render an image if a renderer is available.** If `mmdc` (mermaid-cli) is on PATH (or `npx -y @mermaid-js/mermaid-cli`), **also** render the block to an image and reference its path so users without a Mermaid-capable viewer can see it too:
   ```bash
