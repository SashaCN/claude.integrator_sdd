...
The three signals (`classify-size` asks one `AskUserQuestion` per signal):
...
- **XS** — ≤1 day, no migration, no new API. (Typo, copy fix, config tweak.)
- **S** — ~1 week, maybe a small migration.
...
On edge cases, name the dominant signal: «this is M because it adds a new API + 1–2 sprints, even though it touches only one module».

> **Why no «PR count» signal.** This fork is tuned for a team that ships a feature as **one commit / one PR**, so PR / commit volume carries no size information here — size is read from scope (time to merge, new module/API/migration, breaking changes) instead. (Upstream SDD had a fourth «PR count» signal; it was removed.)
...
| ADRs (in `docs/adr/`) — `design` / `decide-adr` | 2–4 typical | 5–12 typical |
